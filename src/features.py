...
def build_features_jm_only(
    adj: pd.DataFrame,
    log_rets: pd.DataFrame,
) -> pd.DataFrame:
    """
    Build the Jump Model feature matrix **without** AR columns.

    Used for the 2×2 grid: JM trend/regime signal is independent of the
    Absorption Ratio fragility signal, which is applied separately at
    backtest time.

    Returns
    -------
    pd.DataFrame with columns:
        ['dd_10', 'sortino_20', 'sortino_60', 'vol_ratio', 'breadth']
    """
    spy_excess = log_rets['SPY']
...
def build_features_jm_only_pit(
    log_rets: pd.DataFrame,
    pit_breadth: pd.Series,
) -> pd.DataFrame:
    """
    JM-only feature matrix (no AR) with PIT breadth instead of RSP/SPY.

    Used by v2_final: AR stays a separate backtest axis (2x2 grid).
    """
    spy_excess = log_rets['SPY']
    ...
    feature_df = pd.concat(
        [dd_10, sortino_20, sortino_60, vol_ratio, pit_breadth.rename('breadth')],
        axis=1,
    )
    feature_df.columns = [
        'dd_10', 'sortino_20', 'sortino_60', 'vol_ratio', 'breadth',
...
def build_features_pit(
    ...
    """
    7-feature JM matrix using PIT stock panel for AR + breadth (v_final).

    SPY microstructure features match ``build_features``; breadth comes from
    the PIT universe instead of RSP/SPY.
    """
    spy_excess = log_rets['SPY']
    ...
    feature_df = pd.concat(
        [
            ...
            sortino_60,
            ar.rename('ar'),
            ar_zscore.rename('ar_zscore'),
            vol_ratio,
            pit_breadth.rename('breadth'),
        ],
        axis=1,
    )
    feature_df.columns = [
